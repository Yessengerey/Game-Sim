...
public:
	...
	//Generate points
	static Mesh* GeneratePoints(Vector3);

	//Generate circle
	static Mesh* GenerateCircle(Vector3, Vector4, float);
	static Mesh* GenerateHalfCircle(Vector3, Vector4, float);
	//Generate line
	static Mesh* GenerateLine(Vector3, Vector3); 
	static Mesh* GenerateArrow(Vector3, Vector3);
	//static Mesh* GeneratePowerBar()

	//Generates a single triangle, with RGB colours
	static Mesh*	GenerateTriangle();
	static Mesh*	LoadMeshFile(const string &filename);

	static Mesh*	GenerateTriangleStrip(const Vector4, const Vector4, const Vector4, vector<Vector3>);
